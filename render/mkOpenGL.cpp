...
internal void R_OpenGL_Init()
{
    ...
    {
        // GLuint queryResultBuffer;
        // openGL->glGenBuffers(1, &queryResultBuffer);
        // openGL->glBindBuffer(GL_QUERY_BUFFER, queryResultBuffer);
        // openGL->glBufferData(GL_QUERY_BUFFER, kilobytes(1), 0, GL_DYNAMIC_READ);
        // openGL->queryCount = 16;
        // openGL->queryIds   = PushArray(openGL->arena, GLuint, openGL->queryCount);
        // openGL->glGenQueries(openGL->queryCount, openGL->queryIds);

        // OcclusionResult *results = PushArrayNoZero(openGL->arena, OcclusionResult, queryCount);
        //
        // for (u32 i = 0; i < queryCount; i++)
        // {
        //     OcclusionResult &result = results[i];
        //     result.queryId          = queryIds[i];
        //     result.history          = 0;
        // }
    }
    VSyncToggle(1);
...
internal OpenGL *R_Win32_OpenGL_Init(OS_Handle handle)
{
    ...
    if (wglMakeCurrent(dc, rc))
    {
        ...
        Win32GetOpenGLFunction(glGetQueryObjectiv);
...
internal void R_Win32_OpenGL_EndFrame(RenderState *renderState, HDC deviceContext, int clientWidth, int clientHeight)
{
    ...
    // Occlusion culling
    {
        // loop over every aabb, render to the scene
        // for (u32 i = 0; i < openGL->queryCount; i++)
        // {
        //     // Indirect buffer
        ...
        //
        //     openGL->glBeginQuery(GL_SAMPLES_PASSED, openGL->queryIds[i]);
        //     // TODO: strip
                                 ...
                                 GLuint *outBrdfLut)
{
    ...
    openGL->glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, cEnvMapSize, cEnvMapSize);
    openGL->glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, captureRbo);

    // Create output cubemap
    envCubemap = R_OpenGL_AllocTexture(ImageType_Cubemap, cEnvMapSize, cEnvMapSize, GL_RGB16F, GL_RGB, true);
    ...
    glViewport(0, 0, cEnvMapSize, cEnvMapSize);
    glEnable(GL_DEPTH_TEST);
    glDepthFunc(GL_LEQUAL);
    openGL->glBindFramebuffer(GL_FRAMEBUFFER, captureFbo);
    GLint matrix = openGL->glGetUniformLocation(shader, "projection");
    ...
    i32 baseWidth           = cEnvMapSize;
    i32 baseHeight          = cEnvMapSize;
